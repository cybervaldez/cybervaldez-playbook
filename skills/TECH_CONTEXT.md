...
The `{{API_BASE}}` placeholder exists but has no backend. Skills showing API patterns (curl to `/api/*`) are examples for when you add a backend (Express, Fastify, etc.).

**To add API capabilities:**
1. Add backend framework (Express, Fastify)
2. Update `{{SERVER_PORT}}` in PROJECT_CONFIG.md
3. Either:
