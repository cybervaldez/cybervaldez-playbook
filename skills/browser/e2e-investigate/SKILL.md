...
1. **Parse Failure**: Read `{{TESTS_PATH}}/e2e-runs/latest/report.md` to identify failed phases
...
grep -A5 "FAIL" {{TESTS_PATH}}/e2e-runs/latest/report.md
```
...
cat {{TESTS_PATH}}/e2e-runs/latest/server.log | grep -E "(Error|Exception|Traceback|404|500)" -A5

# For server restart issues
cat {{TESTS_PATH}}/e2e-runs/latest/server_restart.log
```

**Screenshots:**
```bash
# List available screenshots
ls {{TESTS_PATH}}/e2e-runs/latest/screenshots/

# View specific screenshot (use Read tool)
# {{TESTS_PATH}}/e2e-runs/latest/screenshots/06-server-restart.png
```
...
curl -sf "{{BASE_URL}}{{API_BASE}}/endpoint" | jq '.'
...
```bash
# For API issues
grep -r "relevant_endpoint" {{WEBUI_PATH}}/server/api/

# For UI issues
grep -r "component_name" {{WEBUI_PATH}}/js/

# For state issues
grep -r "fallback\|default\||| " {{WEBUI_PATH}}/js/

# For persistence issues
grep -r "scan\|index\|startup\|outputs" {{WEBUI_PATH}}/server/
```
...
|----------|----------|---------------------|
| **API/Data** | 500 errors, missing fields, wrong data | `{{WEBUI_PATH}}/server/api/*.py`, server.log |
| **UI Element** | Element not found, wrong visibility | `{{WEBUI_PATH}}/js/*.js`, data-testid attrs |
...
```
{{TESTS_PATH}}/e2e-runs/latest/          <- Symlink to most recent run
...
- [ ] API endpoint code in `{{WEBUI_PATH}}/server/api/`
- [ ] Request/response format

### UI Failures
- [ ] Screenshot for visual state
- [ ] JS errors in server.log (browser console)
- [ ] Component code in `{{WEBUI_PATH}}/js/`
...
# Read: {{TESTS_PATH}}/e2e-runs/latest/screenshots/06-server-restart.png

# Server restart log shows clean startup
cat {{TESTS_PATH}}/e2e-runs/latest/server_restart.log
# No exceptions, server started on port {{SERVER_PORT}}
...
grep -r "scan\|index" {{WEBUI_PATH}}/server/ --include="*.py"

# Found: scan_repair.py handles output discovery
# Check if it scans correctly
grep -r "scan" {{WEBUI_PATH}}/server/scan_repair.py
```
...
### Root Cause
The server's startup scan in `{{WEBUI_PATH}}/server/scan_repair.py` does not
...
### Suggested Fix
1. Check `{{WEBUI_PATH}}/server/scan_repair.py` startup logic
