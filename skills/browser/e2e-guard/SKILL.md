...
test_component_shows_correct_data() {
    ...
    api_total=$(curl -sf "$BASE_URL{{API_BASE}}/data" | jq '.total')
...
test_data_loads_at_startup() {
    ...
    # Check API has data
    api_count=$(curl -sf "$BASE_URL{{API_BASE}}/items" | jq '.total')
...
1. **API files** - Check if endpoint is covered in test files
   ```bash
   grep -l "endpoint_name" {{TESTS_PATH}}/*.sh
   ```

2. **JS files** - Check for corresponding test
   ```bash
   # {{WEBUI_PATH}}/js/modal/foo.js -> {{TESTS_PATH}}/test_foo*.sh
   ls {{TESTS_PATH}}/test_*foo*.sh 2>/dev/null
   ```

3. **TestIDs** - Verify new UI elements have testids
   ```bash
   {{VENV_PYTHON}} list-testids.py --json | grep "component-name"
   ```
...
api_call GET "$BASE_URL{{API_BASE}}/new-endpoint"
...
bash {{TESTS_PATH}}/test_component.sh {{SERVER_PORT}}
...
### Files Changed
- {{WEBUI_PATH}}/js/modal/example.js (UI)
- {{WEBUI_PATH}}/server/api/operations.py (API)
...
### Tests Generated
- {{TESTS_PATH}}/test_operations_new_endpoint.sh (NEW)
...
### API Test Pattern
```bash
api_call METHOD "$BASE_URL{{API_BASE}}/endpoint" '{"json":"body"}'
...
bash {{TESTS_PATH}}/ui/test_component.sh {{SERVER_PORT}}
...
- `{{TESTS_PATH}}/lib/test_utils.sh` - Shell test utilities
