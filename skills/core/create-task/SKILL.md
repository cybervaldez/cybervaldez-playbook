---
name: create-task
description: Plan and implement tasks for the {{PROJECT_NAME}} WebUI with e2e test validation. Use when the user asks to implement a feature, fix a bug, update functionality, or any task that modifies the WebUI codebase.
...
| 7 | **Use project Python** | Use {{VENV_PYTHON}} or project-configured Python |
...
ls {{WEBUI_PATH}}/js/**/*.js
ls {{WEBUI_PATH}}/server/api/*.py

# Find existing tests for the area
ls {{TESTS_PATH}}/test_*.sh

# Get available testids
{{VENV_PYTHON}} list-testids.py --json

# Search for related code
grep -r "pattern" {{WEBUI_PATH}}/
```
...
- JavaScript in `{{WEBUI_PATH}}/js/`
- Python API in `{{WEBUI_PATH}}/server/api/`
...
Write a test at `{{TESTS_PATH}}/test_<feature>.sh` that validates the implementation.

**IMPORTANT:** All tests MUST use the shared library at `{{TESTS_PATH}}/lib/test_utils.sh`.
...
#
# Usage: ./{{TESTS_PATH}}/test_<feature>.sh [--port {{SERVER_PORT}}]
# ============================================================================
...
```bash
chmod +x {{TESTS_PATH}}/test_<feature>.sh
./{{TESTS_PATH}}/test_<feature>.sh
```
...
## Shared Test Library (`{{TESTS_PATH}}/lib/test_utils.sh`)
...
9. **Use project Python** - Use {{VENV_PYTHON}}
...
The `{{TEST_JOB}}` job is designated for testing - its `{{OUTPUTS_PATH}}` folder can be safely deleted and rebuilt.
...
```bash
# GET request
curl -sf http://localhost:{{SERVER_PORT}}{{API_BASE}}/endpoint | jq '.'

# POST request
curl -sf -X POST http://localhost:{{SERVER_PORT}}{{API_BASE}}/endpoint \
  -H "Content-Type: application/json" -d '{"key": "value"}'

# DELETE request
curl -sf -X DELETE http://localhost:{{SERVER_PORT}}{{API_BASE}}/endpoint
```
...
api_call GET "$BASE_URL{{API_BASE}}/endpoint"
...
|---------|---------------|
| PORT handling | `PORT="${1:-{{SERVER_PORT}}}"; [[ "$1" == "--port" ]] && PORT="$2"` |
