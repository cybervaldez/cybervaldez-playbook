# {{PROJECT_NAME}} Testing Conventions
...
| `{{TEST_JOB}}` | | Test job/fixture name |
| `{{SERVER_PORT}}` | | Default server port |
| `{{SERVER_START}}` | | Server startup command |
| `{{VENV_PYTHON}}` | | Python path |
| `{{API_BASE}}` | | API base path |
| `{{WEBUI_PATH}}` | | WebUI source path |
| `{{TESTS_PATH}}` | | Test files path |
| `{{OUTPUTS_PATH}}` | | Generated outputs path |
...
| Test job | Use `{{TEST_JOB}}` job |
| Python | Always `{{VENV_PYTHON}}` |
...
```bash
{{VENV_PYTHON}} list-testids.py              # List all
{{VENV_PYTHON}} list-testids.py --filter "*" # Filter
{{VENV_PYTHON}} list-testids.py --json       # JSON output
```
...
```bash
# Config
curl -s http://localhost:{{SERVER_PORT}}{{API_BASE}}/config | jq '.'

# Standard CRUD pattern
curl -s http://localhost:{{SERVER_PORT}}{{API_BASE}}/resource
curl -s -X PUT http://localhost:{{SERVER_PORT}}{{API_BASE}}/resource/{id} -H "Content-Type: application/json" -d '{...}'
curl -s -X DELETE http://localhost:{{SERVER_PORT}}{{API_BASE}}/resource/{id}

# Queue operations
curl -X POST http://localhost:{{SERVER_PORT}}{{API_BASE}}/queue/add -H "Content-Type: application/json" -d '{...}'
curl -s http://localhost:{{SERVER_PORT}}{{API_BASE}}/queue/status
```
...
**ALWAYS use `{{VENV_PYTHON}}`** - Never system python:

```bash
# CORRECT - Always use project python
{{VENV_PYTHON}} script.py
{{VENV_PYTHON}} -m module.name
...
**ALWAYS use `{{TEST_JOB}}` for testing** - Its outputs can be safely deleted and rebuilt:
...
9. **Always use `{{VENV_PYTHON}}`** - Never system python
10. **Always use `{{TEST_JOB}}` for testing** - Safe to rebuild
